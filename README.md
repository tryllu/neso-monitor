...
Adresy email znajduja się w secrects repozytorium ```RECEIVER_EMAILS```. Adresatów, których należy pominąć w czasie wysyłki można poprzedzić znakiem ```#``` na początku linii. 

Aplikacja wykorzystuje adres Gmail do wysyłania powiadomień, a parametry znajdują się w secrets: ```SMTP_EMAIL```, ```SMTP_PASSWORD```.

Aplikacja umożliwia również wysyłanie powiadomień przez WhatsApp z wykorzystaniem serwisu https://www.callmebot.com/. Aby powiadomienie zostało wysłane należy w linii adresata wprowadzić oddzielony przecinkami MSISDN (bez +) oraz ApiKey.
